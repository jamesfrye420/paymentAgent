...
class PaymentProvider(ABC):
    ...
    def __init__(self, name: str, success_rate: float = 0.9, avg_latency: float = 200):
        ...
        self.network_stats = {
            network.value: {"requests": 0, "failures": 0, "total_time": 0.0}
            for network in CardNetwork
        }
        self.method_stats = {
            method.value: {"requests": 0, "failures": 0, "total_time": 0.0}
            for method in PaymentMethod
        }
        self.region_stats = {
            region.value: {"requests": 0, "failures": 0, "total_time": 0.0}
    ...
    def can_process_transaction(self, transaction: Transaction) -> bool:
        ...
        # Check amount limits
        if not (
            capabilities.min_amount <= transaction.amount <= capabilities.max_amount
        ):
            return False
        ...
        if transaction.payment_instrument:
            if (
                transaction.payment_instrument.method
                not in capabilities.supported_methods
            ):
                return False

            # Check card network for card payments
            if (
                transaction.payment_instrument.method == PaymentMethod.CARD
                and transaction.payment_instrument.network
                and transaction.payment_instrument.network
                not in capabilities.supported_networks
            ):
                return False
        ...
        if (
            ...
            and transaction.customer_info.region not in capabilities.supported_regions
        ):
            return False
    ...
    def process_payment(self, transaction: Transaction) -> Dict[str, Any]:
        ...
        if self._is_rate_limited():
            processing_time = time.time() - start_time
            raise ProviderError(
                self.name, "Rate limit exceeded", ErrorCode.RATE_LIMITED.value
            )
        ...
        if success:
            ...
            raise ProviderError(
                self.name, f"Payment failed: {error_code.value}", error_code.value
            )
    ...
    def _get_adjusted_success_rate(self, transaction: Transaction) -> float:
        ...
        if transaction.payment_instrument and transaction.payment_instrument.network:
            network_score = self.get_network_preference_score(
    ...
    def _select_contextual_error(self, transaction: Transaction) -> ErrorCode:
        ...
        if transaction.payment_instrument and transaction.payment_instrument.network:
            ...
            if network == CardNetwork.AMEX:
                base_errors.extend(
                    [ErrorCode.AUTHENTICATION_FAILED, ErrorCode.BLOCKED_CARD]
                )

            # International networks have region issues
            if network in [CardNetwork.JCB, CardNetwork.UNIONPAY]:
                base_errors.extend(
                    [ErrorCode.REGION_BLOCKED, ErrorCode.CURRENCY_NOT_SUPPORTED]
                )

        # Amount-based errors
        if transaction.amount > 5000:
            base_errors.extend([ErrorCode.INSUFFICIENT_FUNDS, ErrorCode.FRAUD_DETECTED])

        # Payment method specific errors
        if transaction.payment_instrument:
            if transaction.payment_instrument.method == PaymentMethod.DIGITAL_WALLET:
                base_errors.extend(
                    [ErrorCode.WALLET_INSUFFICIENT_BALANCE, ErrorCode.WALLET_SUSPENDED]
                )
    ...
    def _track_request_start(self, transaction: Transaction):
        ...
        if transaction.payment_instrument and transaction.payment_instrument.network:
            self.network_stats[transaction.payment_instrument.network.value][
                "requests"
            ] += 1

        if transaction.payment_instrument:
            self.method_stats[transaction.payment_instrument.method.value][
                "requests"
            ] += 1
    ...
    def _track_request_completion(
        self, transaction: Transaction, success: bool, processing_time: float
    ):
    ...
    def get_health(self) -> ProviderHealth:
        ...
        if self.request_count == 0:
            ...
            current_success_rate = (
                self.request_count - self.failure_count
            ) / self.request_count
            current_avg_latency = (
        ...
        for network, stats in self.network_stats.items():
            if stats["requests"] > 0:
                success_rate = (stats["requests"] - stats["failures"]) / stats[
                    "requests"
                ]

...
class PaymentGateway:
    ...
    def __init__(
        self, routing_strategy: RoutingStrategy = RoutingStrategy.HEALTH_BASED
    ):
        """
        ...
        self.circuit_breakers = {
            name: CircuitBreaker(circuit_config) for name in self.providers.keys()
        }
    ...
    def process_payment(
        ...
        if customer_info is None and customer_id:
            ...
            customer_info = CustomerInfo(
                customer_id=customer_id, risk_level=RiskLevel.LOW
            )
    ...
    def _select_network_optimized(
        self, transaction: Optional[Transaction] = None
    ) -> str:
        """Select provider optimized for the card network."""
        if (
            not transaction
            or not transaction.payment_instrument
            or not transaction.payment_instrument.network
        ):
            return self._select_healthiest_provider(transaction)
        ...
        for name, provider in self.providers.items():
            ...
            if hasattr(
                provider, "can_process_transaction"
            ) and not provider.can_process_transaction(transaction):
                continue
    ...
    def _select_healthiest_provider(
        self, transaction: Optional[Transaction] = None
    ) -> str:
        ...
        for name, provider in self.providers.items():
            ...
            if (
                transaction
                and hasattr(provider, "can_process_transaction")
                and not provider.can_process_transaction(transaction)
            ):
                continue
    ...
    def _select_round_robin(self, transaction: Optional[Transaction] = None) -> str:
        ...
        if transaction:
            ...
            for name in provider_names:
                ...
                if hasattr(
                    provider, "can_process_transaction"
                ) and provider.can_process_transaction(transaction):
    ...
    def _select_failover(self, transaction: Optional[Transaction] = None) -> str:
        ...
        for attempt in range(self.retry_config.max_attempts):
            ...
            try:
                ...
                self.monitor.record_metric(
                    "payment_latency", result.get("processing_time", 0)
                )
            ...
            try:
                ...
                rd = RoutingDecision(
                    ...
                    alternative_providers=[
                        p for p in self.providers.keys() if p != transaction.provider
                    ],
                ...
                route = Route(
                    ...
                    provider_message=result.get(
                        "provider_message", "Transaction approved"
                    ),
                ...
                transaction.metadata.update(
                    {
                        ...
                        "processing_fee": round(
                            transaction.amount * 0.029, 2
                        ),  # Realistic processing fee
                        "network_response_code": network_response_code,
                        "provider_response_code": provider_response_code,
                    }
                )
                ...
                provider_code, network_code = failure_codes.get(
                    error_type, ("ERROR", "96")
                )
    ...
    def _switch_provider(self, transaction: Transaction):
        ...
        available_providers = [
            name for name in self.providers.keys() if name != current_provider
        ]
        ...
        if available_providers:
            ...
            temp_providers = {
                k: v for k, v in self.providers.items() if k != current_provider
            }
    ...
    def simulate_scenario(self, scenario_name: str) -> Dict[str, Any]:
        ...
        """
        scenarios = {
            "stripe_maintenance": lambda: self.configure_provider(
                "stripe", is_maintenance=True
            ),
            "adyen_high_latency": lambda: self.configure_provider(
                "adyen", avg_latency=2000
            ),
            "paypal_low_success": lambda: self.configure_provider(
                "paypal", success_rate=0.3
            ),
            "razorpay_rate_limit": lambda: self.configure_provider(
                "razorpay", rate_limit_threshold=1
            ),
            "mass_failure": lambda: [

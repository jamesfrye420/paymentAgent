...
class StructuredLogger:
    ...
    def __init__(self, log_directory: str = "logs"):
        ...
        self.log_files = {
            ...
            'circuit_breaker_events': self.log_directory / 'circuit_breaker_events.jsonl',
    ...
    def log_payment_event(self, event: PaymentEvent, additional_context: Dict[str, Any] = None):
        ...
        log_entry = StructuredLogEntry(
            ...
            context={
                ...
                'payment_method': transaction.payment_instrument.method.value if transaction.payment_instrument else None,
                'card_network': transaction.payment_instrument.network.value if transaction.payment_instrument and transaction.payment_instrument.network else None,
                'customer_region': transaction.customer_info.region.value if transaction.customer_info and transaction.customer_info.region else None,
                'customer_risk_level': transaction.customer_info.risk_level.value if transaction.customer_info else None,
    ...
    def log_routing_decision(self, transaction: Transaction, decision_factors: Dict[str, Any], 
                           selected_provider: str, alternatives: List[str]):
        ...
        log_entry = StructuredLogEntry(
            ...
            routing_context={
                'provider_health_scores': decision_factors.get('provider_health_scores', {}),
                'network_compatibility': decision_factors.get('network_compatibility', {}),
                'cost_analysis': decision_factors.get('cost_analysis', {}),
                'historical_performance': decision_factors.get('historical_performance', {}),
                'regional_preferences': decision_factors.get('regional_preferences', {})
            }
        )
        
        self._write_log(log_entry, 'routing_decisions')
    
    def log_failure_analysis(self, transaction: Transaction, error_code: str, 
                           error_message: str, failure_context: Dict[str, Any]):
        ...
        log_entry = StructuredLogEntry(
            ...
            context={
                ...
                'payment_context': {
                    ...
                    'payment_method': transaction.payment_instrument.method.value if transaction.payment_instrument else None,
                    'card_network': transaction.payment_instrument.network.value if transaction.payment_instrument and transaction.payment_instrument.network else None,
                    'issuer': transaction.payment_instrument.issuer if transaction.payment_instrument else None,
                    'country': transaction.payment_instrument.country_code if transaction.payment_instrument else None
                },
                'attempt_history': [route.to_dict() for route in transaction.route_history],
            ...
            error_details={
                ...
                'similar_failures_count': failure_context.get('similar_failures_count', 0)
            }
        )
        
        self._write_log(log_entry, 'failure_analysis')
    
    def log_circuit_breaker_event(self, provider: str, state_change: str, 
                                 failure_count: int, context: Dict[str, Any]):
    ...
    def _extract_routing_context(self, transaction: Transaction) -> Dict[str, Any]:
        ...
        return {
            ...
            'providers_tried': list(set(route.provider for route in transaction.route_history)),
            'routing_decisions': [
                route.routing_decision.to_dict() if route.routing_decision else None 
                for route in transaction.route_history
            ],
            'provider_switches': len(set(route.provider for route in transaction.route_history)) - 1
        }
    
    def _extract_performance_metrics(self, transaction: Transaction) -> Dict[str, float]:
        """Extract performance metrics from transaction."""
        if not transaction.route_history:
            return {}
        
        processing_times = [
            route.processing_time for route in transaction.route_history 
        ...
        return {
            ...
            'successful_attempts': len([r for r in transaction.route_history if r.status == 'success']),
            'failed_attempts': len([r for r in transaction.route_history if r.status == 'failed'])
        }
    ...
    def _calculate_business_impact(self, event: PaymentEvent) -> Dict[str, Any]:
        ...
        impact = {
            'revenue_at_risk': transaction.amount if event.event_type.endswith('failure') else 0,
            ...
            'cost_implications': {
                ...
                'retry_costs': len(transaction.route_history) * 0.01,  # Estimated retry cost
                'opportunity_cost': transaction.amount * 0.1 if event.event_type.endswith('failure') else 0
            }
        }
    ...
    def get_logs_for_analysis(self, log_type: str = None, 
                            since: datetime = None, 
                            limit: int = None) -> List[Dict[str, Any]]:
        """Retrieve logs for analysis."""
        logs = []
        
        log_files_to_read = [self.log_files[log_type]] if log_type else self.log_files.values()
    ...
    def export_logs_for_llm_training(self, output_file: str, 
        ...
        if include_patterns:
            filtered_logs = []
            for log in all_logs:
                if any(pattern in log.get('event_type', '') for pattern in include_patterns):
    ...
    def generate_failure_patterns_report(self) -> Dict[str, Any]:
        ...
        for log in failure_logs:
            ...
            patterns['error_code_frequency'][error_code] = patterns['error_code_frequency'].get(error_code, 0) + 1
            
            # Provider failure rates
            patterns['provider_failure_rates'][provider] = patterns['provider_failure_rates'].get(provider, 0) + 1
            
            # Network specific failures
            network = log.get('context', {}).get('payment_context', {}).get('card_network')
            if network:
                if network not in patterns['network_specific_failures']:
                    patterns['network_specific_failures'][network] = {}
                patterns['network_specific_failures'][network][error_code] = patterns['network_specific_failures'][network].get(error_code, 0) + 1
            
            # Time-based patterns
            hour = datetime.fromisoformat(log['timestamp']).hour
            patterns['time_based_patterns'][hour] = patterns['time_based_patterns'].get(hour, 0) + 1
            
            # Amount-based patterns
            amount = log.get('context', {}).get('payment_context', {}).get('amount', 0)
            amount_range = f"{int(amount//100)*100}-{int(amount//100)*100+99}"
            patterns['amount_based_patterns'][amount_range] = patterns['amount_based_patterns'].get(amount_range, 0) + 1

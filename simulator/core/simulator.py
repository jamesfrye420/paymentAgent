...
class RealisticPaymentSimulator:
    ...
    def _calculate_risk_score(self, customer: CustomerInfo, transaction_amount: float, 
                            payment_instrument: PaymentInstrument, merchant: Dict[str, Any]) -> float:
        ...
        if payment_instrument.method.name == 'CARD':
            ...
            if payment_instrument.network and payment_instrument.network.name in high_risk_networks:
                risk_score += 5
            
            # Expired card check
            current_year = datetime.now().year
            current_month = datetime.now().month
            if (payment_instrument.expiry_year and payment_instrument.expiry_month and
                (payment_instrument.expiry_year < current_year or 
                (payment_instrument.expiry_year == current_year and payment_instrument.expiry_month < current_month))):
                risk_score += 15
    ...
    def _should_inject_failure(self) -> bool:
        ...
        if ((7 <= hour <= 9) or (12 <= hour <= 14) or (17 <= hour <= 19)):
            ...
            enhanced_probability = max(enhanced_probability, pattern.get('probability', base_probability))
    ...
    def _inject_contextual_failure(self, transaction_amount: float = 0, customer_region: str = ""):
    ...
    def _generate_fraud_indicators(self, risk_score: float, customer: CustomerInfo, 
                              payment_instrument: PaymentInstrument) -> List[str]:
    ...
    def run_high_failure_simulation(self):
        ...
        for provider_name in ['stripe', 'adyen', 'paypal', 'razorpay']:
            self.gateway.configure_provider(provider_name, success_rate=0.6)  # 60% success instead of 85-90%
        
        # Inject multiple concurrent failures
        failure_scenarios = ['stripe_maintenance', 'paypal_low_success', 'adyen_high_latency']

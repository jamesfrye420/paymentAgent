    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        try:
            result = gateway.process_payment(
                amount=scenario["amount"], currency=scenario["currency"]
            )
            ...
            if transaction["route_history"]:
                print(
                    f"   Processing time: {transaction['route_history'][-1].get('processing_time', 'N/A'):.3f}s"
                )
...
def demo_routing_strategies():
    ...
    for strategy, description in strategies:
        ...
        for i in range(8):
            try:
                result = gateway.process_payment(
                    amount=100.0 + (i * 10), currency="USD"
                )
    ...
    for scenario_name, description in failure_scenarios:
        ...
        for i in range(3):
            try:
                ...
                transaction = result["transaction"]
                print(
                    f"   Payment ${amount}: {transaction['status']} via {transaction['provider']}"
                )

                # Show retry attempts
                if len(transaction["route_history"]) > 1:
                    print(
                        f"   -> Required {len(transaction['route_history'])} attempts"
                    )
                    for j, route in enumerate(transaction["route_history"]):
                        print(
                            f"      Attempt {j+1}: {route['provider']} -> {route['status']}"
                        )
...
def demo_provider_health():
    ...
    for provider, health in health_data.items():
        ...
        print(
            f"   Circuit Breaker: {health.get('circuit_breaker', {}).get('state', 'UNKNOWN')}"
        )
...
def demo_metrics():
    ...
    for i in range(10):
        try:
            gateway.process_payment(
                amount=200.0, currency="USD", preferred_provider="paypal"
            )
        except:
            pass
